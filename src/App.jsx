...
const dropdownStyle = { position: 'absolute', top: '45px', left: 0, width: '100%', backgroundColor: '#fff', border: '1px solid #ccc', zIndex: 10, maxHeight: '80px', overfl      esse codigo deu esse erro ==> Usando a versão 22.22.0 do Node.js (padrão) 
...
Atualizado, auditado, 149 pacotes em 2s
Dez projetos estão buscando financiamento.
  Execute `npm fund` para obter detalhes.
12 vulnerabilidades de gravidade moderada
Para resolver problemas que não exigem atenção, execute:
  correção de auditoria do npm
Para resolver todos os problemas (incluindo alterações que quebram a compatibilidade), execute:
  npm audit fix --force
Execute `npm audit` para obter detalhes.
> pragendar@0.0.1 build
> construção vite
vite v5.4.21 compilando para produção...
transformando...
✓ 3 módulos transformados.
A compilação falhou em 51ms.
Erro durante a compilação:
[vite:esbuild] A transformação falhou com 1 erro:
/opt/render/project/src/src/App.jsx:828:195: ERRO: Literal de string não finalizada
arquivo: /opt/render/project/src/src/App.jsx:828:195
literal de string não terminada
826| <input type="time" value={horarioAbertura} onChange={e => setHorarioAbertura(e.target.value)} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px', border: '1px solid #ccc', boxSizing: 'border-box' }} />
827| <label style={{ fontSize: '12px', fontWeight: 'bold' }}>🕐 Fechamento</label>
828| <input type="time" value={horarioFechamento} onChange={e => setHorarioFechamento(e.target.value)} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px
   | ^
829|  
    em failureErrorWithLog (/opt/render/project/src/node_modules/esbuild/lib/main.js:1472:15)
    em /opt/render/project/src/node_modules/esbuild/lib/main.js:755:50
    em responseCallbacks.<computado> (/opt/render/project/src/node_modules/esbuild/lib/main.js:622:9)
    em handleIncomingPacket (/opt/render/project/src/node_modules/esbuild/lib/main.js:677:12)
    em Socket.readFromStdout (/opt/render/project/src/node_modules/esbuild/lib/main.js:600:7)
    em Socket.emit (node:events:519:28)
    em addChunk (node:internal/streams/readable:561:12)
    em readableAddChunkPushByteMode (node:internal/streams/readable:512:3)
    em Readable.push (node:internal/streams/readable:392:5)
    em Pipe.onStreamRead (node:internal/stream_base_commons:189:23)
Menu
==> A compilação falhou 😞
==> Métodos comuns para solucionar problemas de implantação: https://render.com/docs/troubleshooting-deploys deu esse erro com esse codigo  import { useEffect, useState } from "react";
          ...
          <section style={{ padding: '15px', borderRadius: '10px', border: '1px solid #eee', marginBottom: '15px', backgroundColor: '#fcfcfc' }}>
            ...
            {!editingProfile ? (
              ...
              <div>
                ...
                <input type="time" value={horarioFechamento} onChange={e => setHorarioFechamento(e.target.value)} style={{ width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '5px     era pra ficar tipo esse mais sem o erro Linhas totais: 1.047 linhas (confirmado)
